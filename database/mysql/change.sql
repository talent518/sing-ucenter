...
CREATE TABLE `user_integral_log` (
  ...
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`user_id`,`periods_id`,`dest_type`,`dest_id`,`flag`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
/*!50100 PARTITION BY HASH (user_id)
PARTITIONS 128 */;
...
CREATE TABLE `user_watch_time_element` (
  ...
  `is_complete` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否已完成',
  `is_playable` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否可播放',
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`user_id`,`periods_id`,`class_id`,`textbook_id`,`segment_id`,`element_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
/*!50100 PARTITION BY HASH (user_id)
PARTITIONS 128 */;
