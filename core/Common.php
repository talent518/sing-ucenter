...
class Common
{
    ...
    {
        return sprintf('%s in %s line %d,trace:%s', $ex->getMessage(),
    ...
    public static function getMicroTime(): int
    {
        ...
        return (int)(((float)$micro + (float)$sec) * 1000);
    }
     ...
     */
    public static function pushAsyncJob(JobInterface $job)
    {
        $queueName = 'queue' . ((self::getMicroTime() % 4) + 1);
        \Yii::$app->$queueName->push($job);
    }
}